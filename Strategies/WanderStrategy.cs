...
public sealed class WanderStrategy : IMovementStrategy
{
    ...
    public void Move(Enemy enemy, float deltaTime)
    {
        ...
        // Bounce off walls
        if (enemy.X < 0)              { enemy.X = 0;                              _angle = MathF.PI - _angle; }
        if (enemy.X > GameConstants.CanvasWidth  - enemy.Width)  { enemy.X = GameConstants.CanvasWidth  - enemy.Width;  _angle = MathF.PI - _angle; }
        if (enemy.Y < 0)              { enemy.Y = 0;                              _angle = -_angle; }
        if (enemy.Y > GameConstants.CanvasHeight - enemy.Height) { enemy.Y = GameConstants.CanvasHeight - enemy.Height; _angle = -_angle; }
    }
}
